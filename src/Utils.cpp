...
/*void Duale (const PolyhedralMesh& base, PolyhedralMesh& duale) {
	...
	// Create all Vertexes (Cell0Ds) of duale
	duale.NumCell0Ds = base.NumCell2Ds;
	duale.Cell0DsId = base.Cell2DsId;
	duale.Cell0DsCoordinates.resize(duale.NumCell0Ds, 3);

	for (int f = 0; f < base.NumCell2Ds; ++f) {
		std::vector<int> v;
		bool badVer = false;
		for (int j = 0; j < base.Cell2DsVertices.cols(); ++j) {
			int VerId = base.Cell2DsVertices(f, j);
			if (VerId < 0) {
		...
		if (badVer || v.empty()) continue;

		Eigen::RowVector3d sum(0.0,0.0,0.0);
		for (int idx:v) {
			sum += base.Cell0DsCoordinates.row(idx);
		}

		Eigen::RowVector3d centroid = sum/static_cast<double>(v.size());

		duale.Cell0DsCoordinates.row(f) = centroid;
	}

	// Create all Edges (Cell1Ds) of duale --- TO COMPLETE
	duale.NumCell1Ds = base.NumCell1Ds;
	duale.Cell1DsId.resize(duale.NumCell1Ds);
	duale.Cell1DsId = base.Cell1DsId;
	duale.Cell1DsExtrema.resize(duale.NumCell1Ds, 2);

	for (int f = 0; f < base.NumCell1Ds; ++f) {
		std::vector<int> faces;
		for (int j = 0; j < base.NumCell2Ds; ++j) {
			for (int q = 0; q < base.Cell2DsEdges.cols(); ++q) {
				if (base.Cell2DsEdges(j, q) == f) {
					faces.push_back(j);
				}
			}
		}
		...
		if (faces.size() != 2) {
			cerr << "Something went wrong, edge with only " << faces.size() << " adjacent face(s)" << endl;
			assert(false);
		}

		if (faces.size() != 2) {
			cerr << "Edge " << f << " has " << faces.size() << " adjacent faces. Faces: ";
			for (auto face : faces) cerr << face << " ";
			cerr << endl;
//			assert(false);
		}

		duale.Cell1DsExtrema.row(f) = Eigen::Vector2i(faces[0], faces[1]);
	}


	// Create all Faces (Cell2Ds) of duale --- TO START
//	FOR LATER
